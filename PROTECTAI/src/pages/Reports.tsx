...
export default function Reports() {
  ...
  const generateSimulationReport = () => {
    if (!reports || generating) return;
    setGenerating(true);
    window.setTimeout(() => {
      const report: ReportItem = {
        id: Date.now(),
        title: stage === 'response' ? 'Preliminary Incident Report · HW-104' : 'Simulation Risk Assessment · HW-104',
        type: stage === 'response' ? 'incident_response' : 'risk_intelligence',
        period: 'Current simulation',
        generated_by: 'Protect AI Copilot',
        status: 'ready',
        summary: stage === 'response' ? 'Evidence snapshot, response actions and initial regulatory reporting context for the simulated event.' : 'Compound permit-risk assessment with sensor, spatial, historical and compliance evidence.',
        generated_at: new Date().toISOString(),
      };
      setReports((current) => current ? [report, ...current] : [report]);
      setGenerating(false);
    }, 450);
  };
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <button onClick={generateSimulationReport} disabled={generating} className="flex h-9 items-center gap-2 rounded-lg bg-gradient-to-b from-cyan-500 to-sky-600 px-3 text-xs font-semibold text-zinc-950 shadow-lg shadow-cyan-500/10 hover:from-cyan-400 hover:to-sky-500 disabled:cursor-not-allowed disabled:opacity-60">
              <Plus className={`h-3.5 w-3.5 ${generating ? 'animate-spin' : ''}`} />
              {generating ? 'Generating…' : stage === 'response' ? 'Generate incident report' : 'New report'}
            </button>
          </>
        }
      />
