...
import { searchEvidence, type EvidenceRecord } from '../lib/evidence';
...
export default function AICopilot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: 'assistant',
      content:
        "Hi, I'm the Protect AI Copilot. I can answer using the Protect AI simulation evidence corpus, including incidents, permits, sensor context and safety guidance. Try one of the suggested prompts below.",
  ...
  const [evidence, setEvidence] = useState<EvidenceRecord[]>(() => searchEvidence('hot work gas permit', 4));
  ...
  const send = async (text: string) => {
    ...
    setInput('');
    setEvidence(searchEvidence(trimmed, 4));
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="AI Copilot"
        subtitle="Evidence-backed answers over the simulation incident, permit, sensor and safety-guidance corpus."
        meta={
          <span className="font-mono text-[11px] text-zinc-500">
            Retrieval: <span className="text-zinc-300">prototype evidence search</span> · v0.2
          </span>
        }
        actions={
          <>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1fr_320px]">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-mono text-[10.5px] text-zinc-500">
                  {sending ? 'Searching evidence…' : 'Ready · evidence corpus connected'}
                </p>
              </div>
            </div>
            <button
                ...
                setMessages([
                  {
                    ...
                    role: 'assistant',
                    content:
                      "Hi, I'm the Protect AI Copilot. I can answer using the Protect AI simulation evidence corpus, including incidents, permits, sensor context and safety guidance.",
                    ts: new Date().toISOString(),
                  },
                ])
              }
        ...
        <aside className="space-y-4">
          ...
          <div className="surface-glass rounded-xl p-4">
            ...
            <p className="mt-1 text-[11px] text-zinc-500">Sources currently supporting the conversation.</p>
            <div className="mt-3 space-y-2">
              {evidence.length ? evidence.map((item) => (
                <div key={item.id} className="rounded-lg border border-zinc-800/60 bg-zinc-900/35 p-2.5">
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-mono text-[10px] text-violet-300">{item.id}</span>
                    <span className="text-[9px] uppercase tracking-wider text-zinc-600">{item.kind.replace('_', ' ')}</span>
                  </div>
                  <p className="mt-1 text-[11px] font-medium text-zinc-200">{item.title}</p>
                  <p className="mt-1 line-clamp-2 text-[10.5px] leading-relaxed text-zinc-500">{item.excerpt}</p>
                </div>
              )) : <p className="text-xs text-zinc-500">No matching evidence found. Try a zone, permit or activity.</p>}
            </div>
            <div className="mt-3 flex items-center gap-2 text-[10.5px] text-emerald-300"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />Every answer is traceable to a source record.</div>
          </div>
        </aside>
      </div>
    </div>
